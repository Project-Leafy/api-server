...
public record PlantIdRequestDto(
        ...
        String health,
        @JsonProperty("api_key") String apiKey // ⭐️ details만 제거하고 apiKey는 유지
        // List<String> details <-- 이 필드를 삭제합니다.
) {
    // 생성자도 필드 개수에 맞게 수정해야 합니다.
    public PlantIdRequestDto(List<String> images, Double latitude, Double longitude, String health, String apiKey) {
        this(images, latitude, longitude, true, health, apiKey);
    }
}