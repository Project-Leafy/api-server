...
-- 설명: user_id가 1인 데이터가 이미 있으면(충돌나면) 아무것도 하지 않음(DO NOTHING)
INSERT INTO users (user_id, email, nickname, oauth_provider, current_plants_count, onboarding_status, role, created_at, updated_at)
VALUES (1, 'oksorry12@naver.com', '권준희', 'kakao', 2, false, 'USER', NOW(), NOW())
...
-- 1번: 'choco'
INSERT INTO my_plant (plant_id, user_id, species_id, nickname, adoption_date, image_url, last_watered_date, status_code, created_at, updated_at)
VALUES (1, 1, 101, 'choco', '2024-01-15', 'https://leafy-s3-bucket.s3.ap-northeast-2.amazonaws.com/leafy_test_2.jpg', '2024-05-20', 'HEALTHY', NOW(), NOW())
ON CONFLICT (plant_id) DO NOTHING;

-- 2번: 'banana'
INSERT INTO my_plant (plant_id, user_id, species_id, nickname, adoption_date, image_url, last_watered_date, status_code, created_at, updated_at)
VALUES (2, 1, 102, 'banana', '2024-03-10', 'https://leafy-s3-bucket.s3.ap-northeast-2.amazonaws.com/leafy_test.jpg', '2024-05-15', 'HEALTHY', NOW(), NOW())
ON CONFLICT (plant_id) DO NOTHING;


-- 4. 시퀀스(Auto Increment) 재설정
-- PostgreSQL에서는 'ON CONFLICT'로 INSERT가 무시되어도, 내부적으로 시퀀스 번호는 증가하지 않을 수 있음.
-- 안전하게 매번 재설정해주는 것이 좋다. (PostgreSQL 10 이상 문법)
